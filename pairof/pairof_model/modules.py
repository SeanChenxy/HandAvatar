...
class ResnetPointnet(nn.Module):
    ...
    def __init__(self, out_dim, hidden_dim, dim=3, final_pool=True, **kwargs):
        super().__init__()
        self.out_dim = out_dim

        self.fc_pos = nn.Linear(dim, 2 * hidden_dim)
        self.block_0 = ResnetBlockFC(2 * hidden_dim, hidden_dim)
        self.use_block1 = kwargs.get('use_block1', True)
        self.block_1 = ResnetBlockFC(2 * hidden_dim, hidden_dim) if self.use_block1 else None
        ...
        self.use_block3 = kwargs.get('use_block3', True)
        self.block_3 = ResnetBlockFC(2 * hidden_dim, hidden_dim) if self.use_block3 else None
        self.block_4 = ResnetBlockFC(2 * hidden_dim, hidden_dim) 
        self.fc_c = nn.Linear(hidden_dim, out_dim)

        self.act = nn.ReLU()
        self.final_pool = final_pool
        if kwargs.get('adapt', 0)>0:
            self.adapt = True
            self.fa = nn.Linear(hidden_dim, kwargs.get('adapt'))
