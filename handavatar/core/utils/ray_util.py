...
from srhand.utils.utils import compute_Jacobian, FastDiff3x3MinvFunction
...
class RaySampler(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def get_z_vals(self, rays_o, rays_d, near, far, model):
        pass


class UniformSampler(RaySampler):
    def __init__(self, near, N_samples, far=1):
        super().__init__(near, far)  # default far is 2*R
    ...
    def get_z_vals(self, rays_o, rays_d, near, far, model):
        ...
        if model.training:
            ...
            t_rand = torch.rand(z_vals.shape).to(near)
...
class ErrorBoundSampler(RaySampler):
    def __init__(self, scene_bounding_sphere, near, N_samples, N_samples_eval, N_samples_extra,
        ...
        self.uniform_sampler = UniformSampler(near, N_samples_eval)
    ...
    def get_z_vals(self, rays_o, rays_d, near, far, model, **kwargs):
        beta0 = model.density.get_beta().detach()

        # Start with uniform sampling
        z_vals = self.uniform_sampler.get_z_vals(rays_o, rays_d, near, far, model)
        ...
        while not_converge and total_iters < self.max_total_iters:
            ...
            # To canonical
            points_flat = model.pts_to_canonical(points_flat, **kwargs)

            # Calculating the SDF only for the new sampled points
            with torch.no_grad():
                samples_sdf = model.implicit_network.module.get_sdf_vals(points_flat)
...
def compute_cardinal_rays(deformer, ps, rays, defconds, batch_inds, phase):
    check = True if phase=='train' else False
    ds = deformer(ps, defconds, batch_inds)
